...
def rate_limit_exception_handler(request: Request, exception: RateLimitExceeded):
    ...
    response = request.app.state.rate_limiter._inject_headers(response, request.state.view_rate_limit)
    return response
